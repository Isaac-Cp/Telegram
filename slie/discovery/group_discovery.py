...
class GroupDiscoveryEngine:
    ...
    async def discover_groups(self):
        ...
        for keyword in self.KEYWORDS:
            logger.info(
                f"[SLIE Discovery] Searching for groups with keyword: {keyword}"
            )
            try:
                result = await telegram_engine.client(
                    functions.contacts.SearchRequest(q=keyword, limit=50)
                )

                for chat in result.chats:
                    if (
                        isinstance(chat, (types.Chat, types.Channel))
                        and hasattr(chat, "username")
                        and chat.username
                    ):
                ...
                logger.error(
                    f"[SLIE Discovery] Error searching for {keyword}: {str(e)}"
                )

        logger.info(
            f"[SLIE Discovery] Total unique groups discovered: {len(all_discovered_links)}"
        )
    ...
    async def _store_discovered_group(self, chat):
        ...
        async with AsyncSessionLocal() as db:
            ...
            await db.commit()
            logger.info(
                f"[SLIE Discovery] Group discovered: {chat.title} ({chat.id})"
            )
