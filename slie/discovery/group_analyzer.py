...
class GroupQualityFilter:
    ...
    async def analyze_group(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            full_chat = await telegram_engine.client(
            ...
            if member_count < self.MIN_MEMBERS:
                logger.info(
                    f"[SLIE Discovery] Rejecting group {chat.title}: Members {member_count} < {self.MIN_MEMBERS}"
                )
                await self._update_group_status(
                    telegram_id, "REJECTED_LOW_MEMBERS"
                )
                return False

            # 2. Check Message Frequency (Last 24 hours)
            messages = await telegram_engine.client.get_messages(
                chat, limit=100
            )
            ...
            if len(msgs_last_24h) < self.MIN_MESSAGES_PER_DAY:
                logger.info(
                    f"[SLIE Discovery] Rejecting group {chat.title}: Msg freq {len(msgs_last_24h)} < {self.MIN_MESSAGES_PER_DAY}"
                )
                await self._update_group_status(
                    telegram_id, "REJECTED_LOW_ACTIVITY"
                )
                return False
            ...
            if isinstance(chat, types.Channel) and chat.broadcast:
                logger.info(
                    f"[SLIE Discovery] Rejecting group {chat.title}: Broadcast channel detected."
                )
                await self._update_group_status(
                    telegram_id, "REJECTED_BROADCAST"
                )
                return False

            # Group is valid
            logger.info(
                f"[SLIE Discovery] Group {chat.title} passed quality filter."
            )
            await self._update_group_status(
                telegram_id, "APPROVED", member_count=member_count
            )
            return True

        except Exception as e:
            logger.error(
                f"[SLIE Discovery] Error analyzing group {telegram_id}: {str(e)}"
            )
            return False

    async def _update_group_status(
        self, telegram_id: int, status: str, member_count: int | None = None
    ):
