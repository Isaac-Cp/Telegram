...
class TelegramClientEngine:
    ...
    async def connect(
        ...
        try:
            session = (
                ...
                if session_str
                else StringSession(self.settings.telegram_session_string)
            )
            ...
            if not await self.client.is_user_authorized():
                logger.warning(
                    "[SLIE Telegram] Client not authorized. Manual intervention required."
                )
                return None

            self.is_running = True
            logger.info("[SLIE Telegram] Client connected and authorized.")
            return self.client

        except errors.FloodWaitError as e:
            logger.error(
                f"[SLIE Telegram] FloodWait: Must wait {e.seconds} seconds."
            )
    ...
    async def join_group(self, invite_link: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[SLIE Telegram] Successfully joined group via {invite_link}"
            )
            return True
        except errors.FloodWaitError as e:
            logger.warning(
                f"[SLIE Safety] Cooldown activated: FloodWait for {e.seconds}s during join."
            )
            return False
        except Exception as e:
    ...
    async def send_reply(
        ...
        try:
            ...
            logger.warning(
                f"[SLIE Safety] Cooldown activated: FloodWait for {e.seconds}s during reply."
            )
            return False
        except Exception as e:
    ...
    async def send_private_message(self, user_id: int, text: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"[SLIE Safety] Cooldown activated: FloodWait for {e.seconds}s during DM."
            )
            return False
        except Exception as e:
    ...
    def add_event_handler(self, callback, event_type):
        ...
        if self.client:
            ...
            logger.info(
                f"[SLIE Telegram] Registered event handler: {callback.__name__}"
            )
