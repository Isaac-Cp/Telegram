...
from slie.models.lead_models import User, LeadLTVScore
...
class LTVScoringEngine:
    ...
    RESELLER_KEYWORDS: ClassVar[list[str]] = [
        "xtream codes",
        "panel access",
        "reseller credits",
        "server management",
    ...
    async def calculate_ltv(self, user_id: str) -> float:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # 7. Store result
            ltv_stmt = select(LeadLTVScore).where(
                LeadLTVScore.user_id == user.id
            )
            ...
            if ltv_record:
                ...
                new_ltv = LeadLTVScore(
                    user_id=user.id, ltv_score=ltv_score, ltv_tier=ltv_tier
                )
                db.add(new_ltv)
            ...
            if ltv_tier in ["RESELLER_POTENTIAL", "HIGH_VALUE_BUYER"]:
                logger.info(
                    f"[SLIE Lead Engine] high value lead detected: {user.username} - LTV: {ltv_score:.2f} ({ltv_tier})"
                )
