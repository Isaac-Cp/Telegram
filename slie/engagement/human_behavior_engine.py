...
class HumanBehaviorEngine:
    ...
    LIMITS: ClassVar[dict[str, dict[str, int]]] = {
        "group_join": {"daily": 2, "hourly": 1},
        "public_reply": {"daily": 15, "hourly": 5},
        "dm": {"daily": 3, "hourly": 1},
    }

    DELAYS: ClassVar[dict[str, tuple[int, int]]] = {
    ...
    async def _get_redis(self):
        if self.redis is None:
            try:
                ...
                logger.warning(
                    f"[SLIE Safety] Redis connection failed, falling back to in-memory: {e}"
                )
    ...
    async def authorize_action(self, action_type: str) -> bool:
        ...
        if daily_count >= self.LIMITS[action_type]["daily"]:
            logger.warning(
                f"[SLIE Safety] cooldown activated: {action_type} - Daily limit reached ({daily_count})."
            )
            return False

        if hourly_count >= self.LIMITS[action_type]["hourly"]:
            logger.warning(
                f"[SLIE Safety] cooldown activated: {action_type} - Hourly limit reached ({hourly_count})."
            )
            return False
        ...
        logger.info(
            f"[SLIE Engagement] message scheduled: {action_type} delayed by {delay_minutes} minutes."
        )
        ...
        if r:
            await r.set(
                f"slie:safety:{action_type}", json.dumps(history), ex=86400
            )
        else:
