...
class SellerDensityDetector:
    ...
    PROMOTIONAL_KEYWORDS: ClassVar[list[str]] = [
        "cheap iptv",
        "buy iptv",
        "trial available",
        "reseller panel",
    ...
    async def analyze_market_saturation(
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not group:
                logger.error(
                    f"[SLIE Market Engine] Group {telegram_group_id} not found in database."
                )
            ...
            stmt = (
                select(Message.body)
                .where(Message.telegram_group_id == telegram_group_id)
            ...
            if total_messages == 0:
                logger.info(
                    f"[SLIE Market Engine] No messages found for group {group.name} to analyze."
                )
                return 0.0, "DISCUSSION_GROUP"
            ...
            for msg_body in messages:
                text = msg_body.lower()
                if any(
                    keyword in text for keyword in self.PROMOTIONAL_KEYWORDS
                ):
                    promo_count += 1
            ...
            analysis_stmt = select(GroupMarketAnalysis).where(
                GroupMarketAnalysis.group_id == group.id
            )
            ...
            if market_type == "SELLER_HUB":
                logger.warning(
                    f"[SLIE Market Engine] seller hub detected: {group.name} (Ratio: {seller_ratio:.2f})"
                )
            else:
                logger.info(
                    f"[SLIE Market Engine] Group analysis complete: {group.name} is {market_type} (Ratio: {seller_ratio:.2f})"
                )
