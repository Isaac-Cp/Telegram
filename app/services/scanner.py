...
class MessageIntelligenceScanner:
    ...
    async def start_scanning(self):
        """
        The real-time scanner used to register a second NewMessage handler, which doubled
        work for every Telegram event. MessageScraper now owns ingestion and this scanner
        stays as a compatibility no-op.
        """
        if self._is_registered:
            return

        self._is_registered = True
        logger.info("Message Intelligence Scanner is in compatibility mode; MessageScraper owns live ingestion.")
