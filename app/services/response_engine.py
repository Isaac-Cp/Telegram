...
class ResponseEngine:
    ...
    async def generate_public_response(self, user_id: str, message_text: str, persona: dict) -> str:
        """
        Elite Module 15: Generate a public technical response with conversation context.
        """
        context = memory_engine.get_ai_context(user_id)
    ...
    async def generate_dm_response(self, user_id: str, message_text: str, persona: dict) -> str:
        """
    ...
    async def generate_private_dm(self, lead: Lead) -> str:
        ...
        context = memory_engine.get_ai_context(lead.user_id)
        
        # Check if high value reseller prospect (Module 16)
        ltv_record = lead.ltv_score_record
        is_reseller = bool(ltv_record and "reseller" in (ltv_record.ltv_tier or "").lower())
        ...
        for lead_id in lead_ids:
            try:
                ...
                with SessionLocal() as db:
                    ...
                    persona = power_upgrades_service.select_persona(lead)
                    response_text = await self.generate_public_response(lead.user_id, lead.message_text, persona)
