...
class SellerDensityDetector:
    ...
    async def analyze_market_saturation(
        ...
        with SessionLocal() as db:
            ...
            messages = (
                db.execute(
                    select(Message.body)
                    .where(Message.telegram_group_id == group.telegram_id)
            ...
            if market_type == "SELLER HUB":
                logger.warning(
                    f"[SLIE Market Engine] seller hub detected: {group.name} - Density: {seller_ratio:.2%}"
                )
            else:
                logger.info(
                    f"[SLIE Market Engine] Group analyzed: {group.name} - Type: {market_type}"
                )
    ...
    def analyze_message_batch(
        ...
        seller_ratio = promo_count / total_messages
        duplicate_promo_ratio = (
            duplicate_count / promo_count if promo_count > 0 else 0
        )
        ...
        if seller_ratio > 0.15:
            ...
            phrase in " ".join(messages).lower() for phrase in BUYER_SIGNALS
        ):
