...
class LeadScoringEngine:
    ...
    async def detect_pain_signals(self, message_text: str) -> dict[str, list[str]]:
        """
        Lightweight keyword detection used to decide whether a message deserves deeper analysis.
        """
        lowered = message_text.lower()
        return {
            "complaints": [term for term in COMPLAINT_KEYWORDS if term in lowered],
            "urgency": [term for term in URGENCY_KEYWORDS if term in lowered],
            "buyer_intent": [
                term
                for term in ["recommend", "provider", "trial", "reseller", "panel", "price", "setup"]
                if term in lowered
            ],
        }
    ...
    async def create_lead(self, user_id: int, username: str, group_id: str, message_text: str, message_id: str = None, ai_analysis: dict = None, pain_signals: dict = None):
        ...
        with SessionLocal() as db:
            ...
            if existing_lead:
                lead = existing_lead
                lead.message_text = message_text
            else:
                lead = Lead(
                    user_id=user.id,
                    group_id=group_id,
            ...
            if ai_analysis:
                ...
                lead.intent_score = ai_analysis.get("intent_score", 0) or 0
        ...
        cached_intent_score = None
        if ai_analysis and isinstance(ai_analysis.get("intent_score"), int):
            cached_intent_score = ai_analysis["intent_score"]

        opp_score, priority = await opportunity_engine.score_lead(lead_id, intent_score=cached_intent_score)
        
        # 3. LTV SCORING (Module 2) - defer in low CPU mode and let the scheduled job recalculate later
        if self.settings.low_cpu_mode:
            ltv_score, ltv_tier = 0.0, "DEFERRED"
        else:
            ltv_score, ltv_tier = ltv_engine.calculate_ltv_score(lead_id)

        # 4. Trigger only the upgrades that still add value after the cached scoring pass
        if not self.settings.low_cpu_mode:
