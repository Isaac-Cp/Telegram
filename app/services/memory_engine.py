...
class ConversationMemoryEngine:
    ...
    def log_message_interaction(
        ...
        with SessionLocal() as db:
            ...
            db.commit()
            logger.info(
                f"SLIE Memory Engine: log_message_interaction - User: {user_id}, Type: {message_type}"
            )
            ...
            last_summary_count = (
                summary.message_count_at_last_summary if summary else 0
            )
            if (
                message_count >= 100
                and (message_count - last_summary_count) >= 100
            ):
                import asyncio

                summary_task = asyncio.create_task(
                    self.generate_conversation_summary(user_id)
                )
                summary_task.add_done_callback(
                    lambda task: logger.debug(
                        "Conversation summary task complete: %s",
    ...
    async def generate_conversation_summary(self, user_id: str):
        ...
        with SessionLocal() as db:
            ...
            history_text = "\n".join(
                [f"{c.message_type}: {c.message_text}" for c in all_history]
            )
            ...
            try:
                content = await ai_service.chat_completion(
                    prompt=prompt, response_format="json_object"
                )
                ...
                db.commit()
                logger.info(
                    f"SLIE Memory Engine: generate_conversation_summary - User: {user_id}"
                )
