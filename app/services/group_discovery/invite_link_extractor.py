...
def extract_invite_links(message_text: str):
    ...
    discovered_links = []
    for pattern in INVITE_LINK_PATTERNS:
...
async def handle_message_for_invite_links(event):
    ...
    if event.message.fwd_from and hasattr(event.message.fwd_from, "from_id"):
    ...
    with SessionLocal() as db:
        for link in invite_links:
            ...
            existing = (
                db.query(Group).filter(Group.invite_link == clean_link).first()
            )
            if not existing:
                ...
                db.add(new_group)
                logger.info(
                    f"SLIE Discovery: Invite link extracted: {clean_link} (Source Group: {source_group_id})"
                )
