...
async def schedule_group_join():
    ...
    with SessionLocal() as db:
        ...
        for group in target_groups:
            ...
            account = await telegram_client_manager.rotate_account(
                "group_join"
            )
            if not account:
                logger.info(
                    "[SLIE Joiner] All accounts reached daily limit. Join scheduler: PAUSED."
                )
                break
            ...
            try:
                ...
                logger.info(
                    f"[SLIE Joiner] Account {account.phone_number} joining group: {group.name}"
                )

                # Resolve target for joining
                target = (
                    group.username if group.username else group.invite_link
                )
                if not target:
                    continue
                ...
                # 5. Join group
                await client(
                    functions.channels.JoinChannelRequest(channel=target)
                )
                ...
                await telegram_client_manager.track_account_limits(
                    account.phone_number, "group_join"
                )
