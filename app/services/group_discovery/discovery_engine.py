...
from app.services.group_discovery.group_analyzer import (
    collect_group_metadata,
    analyze_group_activity,
)
from app.services.group_discovery.authority_scoring import (
...
class GroupDiscoveryEngine:
    ...
    async def run_external_discovery_task(self):
        ...
        logger.info(
            "Starting SLIE Group Discovery Engine: External Sources (Reddit)..."
        )
        from app.models.external_lead import ExternalLead
        from app.services.group_discovery.invite_link_extractor import (
            ...
            for lead in external_leads:
                ...
                for link in links:
                    ...
                    existing = (
                        db.query(Group)
                        .filter(
                            (Group.invite_link == clean_link)
                            | (Group.username == link.split("/")[-1])
                        )
                        .first()
                    )
                    ...
                    if not existing:
                        ...
                        db.add(new_group)
                        logger.info(
                            f"New group discovered via Reddit link: {clean_link}"
                        )
            db.commit()
    ...
    async def run_group_analysis_task(self):
        ...
        with SessionLocal() as db:
                ...
                db.execute(
                    select(Group).where(
                        and_(
                            ...
                            (Group.status == "new")
                            | (Group.last_scanned <= twelve_hours_ago),
                        )
                    )
                )
                .scalars()
            ...
            for group in target_groups:
                ...
                db.commit()
                logger.info(
                    f"Group {group.name} analyzed. Status: {group.status}, Authority Score: {group.authority_score}"
                )
